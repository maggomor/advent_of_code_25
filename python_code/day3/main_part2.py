...
class DayThreeClass(BaseModel):
    ...
    def process_battery_bank(self, battery_input: str) -> int:
        ...
        first_val = argmax(result[:-12])
        val_list = [first_val]
        for i in range(10,0,-1):
            val_list.append(val_list[-1]+1+argmax(result[val_list[-1]+1:-i]))
        val_list.append(val_list[-1]+1+argmax(result[val_list[-1]+1:]))
        final_val = int("".join([str(result[x]) for x in val_list]))
        return final_val
