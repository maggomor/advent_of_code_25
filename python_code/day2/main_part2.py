    ...
    def check_repeating_numbers(self, list_input: list) -> int:
        ...
        for number in range(int(list_input[0]), int(list_input[1])+1):
            ...
            for increment in range(1,len(number_str)+1):
                if ((not flag) and (number_str[0:increment] * int((len(number_str) / increment)) == number_str)):
                    if int(len(number_str) / increment) > 1:
                        invalid_ids.append(int(number_str))
                        print(number_str[0:increment] * int((len(number_str) / increment)), number_str)
                        print(f"{number_str} is in range {list_input[0], list_input[1]} ")
                        flag = True
        return invalid_ids 
